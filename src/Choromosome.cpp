...
Choromosome::Choromosome(int width, int height, std::vector<int> inTem)
{
    ...
    for (size_t i = 0; i < height; i++)
        for (size_t j = 0; j < width; j++) piecesArrangment[i][j] = inTem[inx++];
}
    ...
    for (int i = 0; i < height; i++)
        for (int j = 0; j < width; j++)
        {
            if (piecesArrangment[i][j] != -1)
            {
                ...
                {
                    freeBounderiesPositions.push_back(
                        SpatialRelation(SpatialRelation::Up, pieceInx, choroRowInx, choroColInx));
                }
                ...
                {
                    freeBounderiesPositions.push_back(
                        SpatialRelation(SpatialRelation::Down, pieceInx, choroRowInx, choroColInx));
                }
                ...
                {
                    freeBounderiesPositions.push_back(
                        SpatialRelation(SpatialRelation::Left, pieceInx, choroRowInx, choroColInx));
                }
                ...
                {
                    freeBounderiesPositions.push_back(
                        SpatialRelation(SpatialRelation::Right, pieceInx, choroRowInx, choroColInx));
                }
...
void Choromosome::calculateFitness(const std::vector<Piece>& pieces)
{
    ...
    for (size_t i = 0; i < height - 1; i++)
        for (size_t j = 0; j < width - 1; j++)
        {
            ...
            //            std::cout << "current index: "<<currentInx<<" right index: " <<rightInx << " down index:
            //            "<<downInx << std::endl;
...
// check
double Choromosome::getFitness() const { return fitness; }
int Choromosome::getOccupiedPositions() const { return occupiedPosition; }
bool Choromosome::checkPieceAvailability(int pieceId) const { return availabalePieces[pieceId]; }
// sort based on operator < over fitness parameter
bool Choromosome::operator<(const Choromosome& rhs) const { return this->fitness < rhs.getFitness(); }
void Choromosome::shiftLeft()
{
    for (size_t i = 0; i < height; i++)
        for (size_t j = 1; j < width; j++) piecesArrangment[i][j - 1] = piecesArrangment[i][j];

    for (size_t i = 0; i < height; i++) piecesArrangment[i][width - 1] = -1;
}

void Choromosome::shiftRight()
{
    for (int i = 0; i < height; i++)
        for (int j = width - 2; j >= 0; j--) piecesArrangment[i][j + 1] = piecesArrangment[i][j];

    for (size_t i = 0; i < height; i++) piecesArrangment[i][0] = -1;
}

void Choromosome::shiftUp()
{
    for (size_t i = 1; i < height; i++)
        for (size_t j = 0; j < width; j++) piecesArrangment[i - 1][j] = piecesArrangment[i][j];

    for (size_t j = 0; j < width; j++) piecesArrangment[height - 1][j] = -1;
}

void Choromosome::shiftDown()
{
    for (int i = height - 2; i >= 0; i--)
        for (int j = 0; j < width; j++) piecesArrangment[i + 1][j] = piecesArrangment[i][j];

    for (size_t j = 0; j < width; j++) piecesArrangment[0][j] = -1;
}
...
void Choromosome::printChoromosome() const
{
    for (size_t i = 0; i < height; i++)
    {
        for (size_t j = 0; j < width; j++) std::cout << piecesArrangment[i][j] << " ";
    }
    std::cout << std::endl;
}
...
{
    for (size_t i = 0; i < height; i++)
        for (size_t j = 0; j < width; j++) assert(piecesArrangment[i][j] != -1);
}
